...
En la segunda línea, se comete un error de tipeo `greetign = {};`, pero como la variable no esta declarada ni con [var](#glos-var), [let](#glos-let) o [const](#glos-const); Javascript tras bambalinas hace algo como lo siguiente aunque el programador no lo vea:
...
La variable `z` esta dentro del bloque `if` y no deberiamos poder acceder a ella, pero `z` esta declarada con `var`, esto la convierte en una variable con [ambito de función](#glos-ambito_de_funcion) y no de bloque, entonces accedemos a su valor, para poder sumar `x + z` que sería `40`.
...
La primera función en llamarse es `increaseNumber` que solo se encarga de retornar la variable `num` y luego la incrementa; `num` no esta en el scope de la [función](#glos-funcion) por eso pasamos a buscar la variable en el scope global. Esta función regresará `10`.
...
Al llamar a `getStatus` debemos tener el cuenta el [scope](#glos-scope) de las variables (ámbito de las variable en español), recuerda que tanto `let` como `const` tienen scope de bloque, por ende buscara una variable `status` dentro del bloque de `data` y regresara la cadena `"C"`.
...
La función `getName` intenta imprimir por consola `name` antes de ser declarada, por [hoisting](#glos-hoisting) el interprete de JavaScript hará que `name` entre en lo que se denomina [Temporal Dead Zone](#glos-temporal_dead_zone), una región del código donde la variable esta declarada pero no es posible acceder a ella.
...
Cuando declaramos variables primitivas con `const` estas deben ser como su nombre lo indica valores contantes, por ende no podemos motificar su valor, si intenamos cambiarlo obtendremos un [TypeError](#glos-typeerror).
...
Independientemente de la palabra reservada con la que declaremos una variable (`var`, `let`, `const`), esta tendrá [scope global](#glos-scope_global) siempre y cuando no este dentro de un bloque o dentro de una función. 
