...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, toptitle=1mm, colbacktitle=quarto-callout-caution-color!10!white, opacityback=0, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Dificultad}, bottomrule=.15mm, rightrule=.15mm, coltitle=black, left=2mm, opacitybacktitle=0.6, arc=.35mm, titlerule=0mm, breakable, toprule=.15mm, leftrule=.75mm, colback=white, colframe=quarto-callout-caution-color-frame]
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, toptitle=1mm, colbacktitle=quarto-callout-tip-color!10!white, opacityback=0, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Tip}, bottomrule=.15mm, rightrule=.15mm, coltitle=black, left=2mm, opacitybacktitle=0.6, arc=.35mm, titlerule=0mm, breakable, toprule=.15mm, leftrule=.75mm, colback=white, colframe=quarto-callout-tip-color-frame]
...
Recuerda que los objetos se manejan segun su \textbf{referencia} y no
por su \textbf{valor} como lo hacen los tipos primitivos del lenguaje,
esto significa que al hacer esto:
...
disco duro del ordenador; en JavaScript al ser un lenguaje de alto nivel
no podemos acceder a dichas direcciones como en lenguajes de bajo nivel
como por ejemplo \textbf{lenguaje ensamblador} (aunque en Python si se
pude con la función \texttt{id()} pero este no es un libro de Python).
...
alternativas como por ejemplo \texttt{JSON.stringify}.
...
El método \texttt{hasOwnProperty} propio de los objetos retorna un
\texttt{boolean} dependiendo \textbf{si la key del objeto existe o no}.
...
Esto no funciona como en un objeto, recuerda que un \texttt{set} es como
un tipo de arreglo de valores no repetidos. Por ello \texttt{1}
\texttt{string} no concuerda con \texttt{1} \texttt{number}.
...
Cuando en un objeto tenemos keys repetidas, estas se sobre escriben
...
\texttt{String} es el contructor que tiene JavaScript para gestionar las
cadenas de texto. En el ejemplo se agrega la función
...
\textbf{Sin} usar el \texttt{spread\ operator} tendriamos un objeto
anidado:
...
El método \texttt{entries} del constructor \texttt{Object} regresa un
arreglo anidado donde cada sub arreglo corresponde a la llave y valor
del objeto:
...
\texttt{info} solo tiene la propiedad \texttt{favoriteFood} que apunta
al índice \texttt{0} del arreglo \texttt{food}, por lo tanto
...
El operador \texttt{??=} se llama \textbf{Logical Nullish Assignment} es
un operador de corto circuito moderno que consiste en ejecutar porciones
de código si evaluamos una condición como \textbf{nullish}, osea, como
valor \texttt{null} o \texttt{undefined}.
...
\texttt{undefined} o eliminarla con el operador unario \texttt{delete}.
...
Pese a que el valor de \texttt{voice} es \texttt{undefined} la propiedad
existe como tal dentro del objeto, es por ello que al verificarlo con el
operador \texttt{in} obtenemos \texttt{true}.

Algo diferente pasa cuando eliminamos con \texttt{delete} la propiedad
\texttt{tipe\ of\ music}, esta deja de existir en el objeto, no tiene
ningún tipo de valor, ni siquiera \texttt{undefined}, el objeto quedaría
así:
...
con la \textbf{notación de punto} por ejemplo \texttt{object.value} y
con la \textbf{notación de corchetes} por ejemplo
\texttt{object{[}"value"{]}}.
...
\textbf{Trailing commas} es una peculiaridad de ES2015.
...
Las variables de tipo \texttt{Symbol} son relativamente nuevas y tienen
peculiaridades muy interesantes, una de ellas es la creación de
propiedades ocultas o privadas dentro de los objetos.
...
Desde ES6 es posible usar parámetros por defecto siempre y cuando sean
los últimos declarados en la función.
...
Pero el interprete reconoce la palabra \texttt{return} con el fin de una
expresión, por lo tanto, aunque no lo veas, JavaScript hará esto:
...
No pude ser un \texttt{SyntaxError} porque no se cometio ningún error de
tipeo, el código no esta mal escrito pero tampoco es un código valido.

No puede ser \texttt{ReferenceError} porque no hay problemas de
referencia al intentar acceder a la variable \texttt{name}.

Se genera una excepción de tipo \texttt{TypeError} cuando un valor no es
del tipo esperado, entonces se lanza un
...
La función \texttt{sayHi} regresa una otra función de tipo flecha, dicha
función es anónima y solo devuelve la cadena \texttt{Hi\ JavaScript}, el
detalle acá es que esta función anónima una vez regresada es
