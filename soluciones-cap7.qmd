...
WTF! Cuando vi que hacer esto es posible casi me caigo de la silla. Expliquemos por qué:

Oíste o leíste alguna vez esta frase: **"Todo en JavaScript es un objeto"** Déjame decirte que no es mentira, literalmente todo es un objeto, todo lo que no sea un tipo primitivo en JavaScript es un objeto, desde arreglos, los propios objetos claro, las promesas, y también las **funciones**.
...
```
Este es un comportamiento muy jocoso del lenguaje y está bueno saber que es posible hacer estas cosas aunque no tenga muchos casos de uso.
...
Podemos llamar a la función antes de su declaración, característica que se denomina [hoisting](#glos-hoisting).
...
Desde [ES6](#glos-es6) es posible usar [parámetros por defecto](#glos-parametros_por_defecto) siempre y cuando sean los últimos declarados en la función.

En este caso el parámetro por defecto `num1` es el mismo que el primer parámetro, no hay ningún problema siempre y cuando esté declarado al final de la lista de parámetros de la función.

Pasamos el argumento `10` a la función `sum`, esto significa que `num2` deberá usar su valor por defecto que sería el mismo de `num1`, o sea `10`; entonces `10 + 10` nos da el resultado final `20`.
...
Después de una expresión JavaScript pone automáticamente un punto y coma para indicar al intérprete que dicha expresión finalizó en una línea de código en concreto. Esto se denomina **Inserción automática de punto y coma**.
...
Pero el interprete reconoce la palabra [return](#glos-return) con el fin de una expresión, por lo tanto, aunque no lo veas, JavaScript hará esto:
...
Y ya sabemos que en una función al encontrar la palabra `return` todo el código posterior que pueda haber no se ejecuta, ni siquiera se evalúa, entonces jamás se llegará a hacer la operación `a + b`.

Cuando una función no retorna nada explícitamente, JavaScript hace que el `return` arroje un `undefined` de manera implícita.
...
No pude ser un [SyntaxError](#glos-syntaxerror) porque no se cometio ningún error de tipeo, el código no esta mal escrito pero tampoco es un código valido.

No puede ser [ReferenceError](#glos-referenceerror) porque no hay problemas de referencia al intentar acceder a la variable `name`.

Se genera una excepción de tipo [TypeError](#glos-typeerror) cuando un valor no es del tipo esperado, entonces se lanza un `TypeError: name is not a function!`
...
Por definición una [Higher Order Function](#glos-higher_order_function) es:
...
La función `sayHi` regresa una otra [función de tipo flecha](#glos-funcion_flecha), dicha función es anónima y solo devuelve la cadena `Hi JavaScript`, el detalle acá es que esta función anónima una vez regresada es inmediatamente llamada.

Entonces `sayHi` será igual a la cadena `Hi JavaScript` y en conclusión su `typeof` igual a `string`.

Podríamos ver también este ejemplo si extraemos la función anónima y escribimos una función auxiliar por aparte, de la siguiente manera:
