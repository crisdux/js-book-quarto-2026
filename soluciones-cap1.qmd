...
El objeto [Number](#glos-number) de JavaScript puede convertir los valores de un arreglo a números, pero hay que tener cuidado con tipos `boolean`, `undefined` o `null`.
...
`typeof 1` regresa `"number"`, literalmente la cadena `"number"`, entonces tenemos `typeof "number"` y esto da obviamente `string`.
...
En el ejemplo intentamos convertir `"7*6"` a base `10`, o sea, a base decimal.

`parseInt` toma los valores válidos de izquierda a derecha, dicho esto, solo tomará el valor `7` (el `*` y todo lo que le sigue no es un valor válido para `parseInt`).
...
No debemos confundir el constructor `Number` y `Boolean` por sí mismos, con dichos constructores acompañados de la palabra [new](#glos-new), si hacemos lo siguiente:
...
Al intentar obtener las `keys` del objeto con [Object.keys](#glos-object_keys) obtendremos un arreglo vacío justamente porque el `Symbol` no es un elemento que se pueda enumerar, de esta manera es posible "ocultar" ciertas propiedades de un objeto.
...
El operador de cortocircuito OR (`||`) solo se ejecuta si el primer operando es **falsy**.
...
`undefined` evalúa como **falsy** entonces tendríamos `"0"`.
...
* `"0" || null`:
`"0"` no evalúa como **falsy** entonces no se ejecuta el operador de cortocircuito.
...
Operando tenemos como resultado `0` porque `undefined` es un valor **nullish**.
...
Finalmente `"0"` como cadena no es un valor **falsy** entonces no podemos ejecutar el operador de cortocircuito dando como resultado final `"0"`.
...
El operador de [igualdad estricta](#glos-igualdad_estricta) es muy potente, pero ¿sabías que existe uno aún mejor?
...
Dicho todo esto y conociendo los valores truthy y los valores falsy no debería costarte llegar a que la respuesta correcta es la opción C.
