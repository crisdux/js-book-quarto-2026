...
Con el bucle _for-in_, podemos iterar sobre propiedades **enumerables**. Los enumerables en el arreglo son justamente sus índices. Por ello es `0` `1` `2`.

Con un bucle _for-of_, podemos iterar sobre **iterables**. Un arreglo por definición es un iterable, en cada iteración la variable `item` es igual al elemento sobre el cual se itera en ese momento. Por ello es `"Radiohead"` `"Coldplay"` `"Nirvana"`.
