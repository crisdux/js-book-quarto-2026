...
En JavaScript existen lo que se denomina **valores `falsy`** y son los siguientes:
...
En otras palabras, también es correcto afirmar que al usar el `===` JavaScript no hace **coerciones de tipos**, por ello es ampliamente sugerido usarlo.
...
**Ambas comparaciones devuelven `false`**

**Explicación:**

Si bien `null` y `undefined` son valores `falsy` al momento en que JavaScript haga **coerciones de tipo** pasa algo raro, esto se debe a que tanto `null` como `undefined` sólo son **iguales** a sí mismos y entre ellos:
...
Hacemos una comparación estricta usando el operador `===`, esto significa que compararemos **valores** y **tipos de datos**, `a` y `b` tienen el mismo valor, pero `a` es de tipo `number` y `b` está siendo inicializada usando el constructor `Number`, por ende es un objeto; entonces obtendremos un `false`.
